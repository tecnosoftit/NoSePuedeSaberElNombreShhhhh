...
public partial class uc_ucReservaCircuito : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                string strHoteles = "";
                for (int i = 0; i < rptCabinas.Items.Count; i++)
                {
                    Label lblHotelCabina = (Label)rptCabinas.Items[i].FindControl("lblHotelCabina");
                    if (i < rptCabinas.Items.Count - 1)
                        strHoteles = strHoteles + lblHotelCabina.Text + ", ";
                    else
                        strHoteles = strHoteles + lblHotelCabina.Text;
                }
                ((Label)rptCircuitos.Items[0].FindControl("lblHoteles")).Text = strHoteles;
            }
            catch { }
        }
    }
    ...
    protected void btnContinuar_Click(object sender, EventArgs e)
    {
        ...
        cCar.LimpiarCarrito();
        clsValidaciones.RedirectPaginaIni("Index.aspx", true);
        //clsValidaciones.RedirectPaginaIni(lblUrlRedireccion.Text, true);
    }
